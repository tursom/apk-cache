...
[apk]
enabled = $APK_ENABLED
verify_hash = $APK_VERIFY_HASH
verify_signature = $APK_VERIFY_SIGNATURE
...
if [ -n "$PROXY_ALLOWED_HOSTS" ]; then
	...
	for host in $PROXY_ALLOWED_HOSTS; do
		...
		trimmed=$(echo "$host" | sed 's/^ *//;s/ *$//')
		printf '"%s"' "$trimmed" >>"$CONFIG"
		first=0
	done
	IFS=$old_ifs
	printf ']\n' >>"$CONFIG"
