...
- **特性**:
  ...
  - SOCKS5/HTTP 代理支持
  - 文件级锁管理 (`utils/lockman.go`)
  - 缓存过期策略 (TTL)
  - 自动清理机制 (`cmd/apk-cache/cleanup.go`)
...
- **特性**:
  - LRU 淘汰策略 (`cmd/apk-cache/memory_cache.go`)
  - TTL 过期时间支持
  - 并发安全访问 (sync.Map)
  - 文件大小限制
  - Prometheus 指标监控 (`utils/monitoring.go`)
- **配置选项**:
...
#### 健康检查和自愈机制
- **实现**: 完整的健康检查和故障转移机制 (`cmd/apk-cache/upstream.go`)
- **特性**:
  - 定期上游服务器健康检查
  - 故障服务器自动剔除
  - 健康状态自动恢复
...
#### 请求限流功能
- **实现**: 基于令牌桶算法的完整请求限流系统 (`utils/rate_limiter.go`)
- **特性**:
  - 全局请求频率限制
  - 突发容量支持
  - 豁免路径配置
...
#### APT 包缓存支持
- **实现**: Debian/Ubuntu APT 包缓存功能 (`cmd/apk-cache/cache_apt.go`)
- **特性**:
  - HTTP/HTTPS 代理模式
  - APT 包文件缓存
  - 与 APK 缓存共享基础设施
...
#### HTTP/HTTPS 代理功能
- **实现**: 完整的 HTTP/HTTPS 代理支持 (`cmd/apk-cache/http_proxy.go`)
- **特性**:
  - CONNECT 方法支持
  - 代理身份验证
  - 缓存代理请求
...
#### 多上游服务器支持
- **实现**: 多上游服务器故障转移机制 (`cmd/apk-cache/upstream.go`)
- **特性**:
  - 多个上游服务器配置
  - 自动故障转移
  - 健康状态监控
...
#### 身份验证系统
- **实现**: 代理和管理界面身份验证 (`cmd/apk-cache/handlers.go`, `utils/ip_utils.go`)
- **特性**:
  - Basic Auth 认证
  - IP 白名单支持
  - 反向代理信任配置 (X-Real-IP, X-Forwarded-For)

#### 缓存配额管理
- **实现**: 磁盘缓存空间管理 (`cmd/apk-cache/cache_quota.go`)
- **特性**:
  - LRU/LFU/FIFO 清理策略
  - 自动缓存清理
  - 磁盘空间监控
...
- **目标**: 差异化缓存管理
- **实现**:
  - 基于包大小的缓存策略（小文件高优先级，大文件低优先级）
  - 基于包类型的缓存策略（正则匹配文件名）
  - 基于访问频率的动态调整（热门/冷门文件识别）
  - 自适应策略（综合多种策略）
  - 索引文件保持固定短TTL
- **预期效果**: 优化缓存效率
...
- **目标**: 提供更细粒度的访问控制
- **实现**:
  - IP 白名单/黑名单 (`utils/ip_utils.go` 已实现白名单)
  - 基于路径的访问控制
  - 请求来源验证
- **预期效果**: 增强安全性
- **状态**: 已实现 Basic Auth + IP 白名单
