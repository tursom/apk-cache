...
│   ├── apk-cache/              # 主应用程序
│   │   ├── main.go             # 程序入口（信号生命周期）
│   │   ├── app.go              # App 组装（配置、HTTP 服务、优雅关闭）
│   │   ├── pipeline.go         # 统一请求流水线（三级缓存编排）
│   │   ├── protocol.go         # ProtocolAdapter 接口 + APK/APT/Proxy 适配器
│   │   ├── proxy_tunnel.go     # CONNECT 隧道双向复制
│   │   ├── memory_cache.go     # 内存 LRU 缓存（带 TTL）
│   │   ├── apk_index_service.go    # APKINDEX 加载与包校验
│   │   ├── apk_archive.go      # APK .apk 归档读取（多 member gzip）
│   │   ├── apk_signature.go    # APK RSA 签名验签
│   │   ├── apt_index_service.go    # APT Release/Packages 索引加载与校验
│   │   ├── admin.html          # 管理仪表板 HTML
│   │   ├── *_test.go           # 测试文件
│   │   └── apk_test_helpers_test.go  # 测试辅助工具
│   ├── apt-hash/               # APT 哈希工具
│   └── i18n-analyzer/          # i18n 覆盖分析器
├── internal/
│   ├── config/
│   │   └── config.go           # TOML 配置加载 + 验证
│   ├── upstream/
│   │   ├── upstream.go         # 上游服务器管理 + failover fetcher
│   │   └── transport.go        # HTTP transport（SOCKS5/HTTP 代理）
│   └── policy/
│       └── policy.go           # 细粒度缓存策略
├── utils/
│   ├── monitoring.go           # Prometheus 指标
│   ├── lockman.go              # 文件级锁管理器
│   ├── rate_limiter.go         # 令牌桶限流器
│   ├── ip_utils.go             # IP 验证工具
│   ├── matcher.go              # 路径匹配工具
│   ├── parse_utils.go          # 路径标准化
│   ├── set.go                  # 集合数据结构
│   ├── decompress.go           # 解压辅助
│   ├── apt/                    # APT 包解析
│   │   ├── package.go
...
│       └── locales/
├── doc/
│   └── 20260406 unified-pipeline-refactor.md  # 统一流水线重构说明
├── build.sh                    # 构建脚本（必需）
├── run_test.sh                 # 集成测试脚本
...
- 键值对格式：`slog.Info("message", "key", value)`
- 避免使用 `log.Println` / `log.Printf` / `fmt.Print`
- 使用适当级别：`Debug`（诊断）、`Warn`（异常但可恢复）、`Error`（故障）
- 不在错误消息中暴露内部路径或配置值

### 国际化 (i18n)
- 仅面向用户的字符串（如 HTTP 错误响应、CLI 输出）使用 `i18n.T()`
- 内部日志和诊断消息使用 `slog`，不使用 i18n
- 翻译键在 `utils/i18n/locales/` 目录中
- 移除已失效的翻译键，避免死代码
